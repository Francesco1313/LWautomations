...
function TagChip({ label }: { label: string }) {
  return (
    <span style={{
      ...
      background: 'var(--grey6)', color: 'var(--grey2)',
      whiteSpace: 'nowrap',
    }}>
      {label}
    </span>
  ...
  return (
    <tr
      ...
      style={{
        ...
        background: hovered ? 'var(--grey7)' : '#fff',
        cursor: 'pointer',
      }}
    >
      {/* Checkbox */}
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
              ...
              <button style={{
                ...
                background: '#fff', color: 'var(--grey2)',
                border: '1px solid var(--grey5)', borderRadius: 4, fontSize: 14, cursor: 'pointer',
              }}>
                Bulk actions ▾
              </button>
              <button style={{
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  placeholder="Search"
                  style={{
                    height: 32, padding: '0 32px 0 10px', width: 220,
                    border: '1px solid var(--grey5)', borderRadius: 4,
                    fontSize: 13, outline: 'none', background: '#fff',
                  }}
                />
                <span style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', color: 'var(--grey4)', fontSize: 13 }}>⌕</span>
              </div>
              {/* DEV: use <FilterDropdown> */}
              {[['Products', 'all'], ['Date', 'all']].map(([label]) => (
                <select key={label} style={{
                  height: 32, padding: '0 10px',
                  border: '1px solid var(--grey5)', borderRadius: 4,
                  fontSize: 13, color: 'var(--grey3)', background: '#fff', cursor: 'pointer',
                }}>
                  <option>{label}</option>
                </select>
              ))}
              ...
              <button style={{
                height: 32, padding: '0 10px',
                border: '1px solid var(--grey5)', borderRadius: 4,
                fontSize: 13, color: 'var(--teal)', background: '#fff', cursor: 'pointer',
              }}>
                + tag filter
              </button>
            </div>
          ...
          <div style={{ background: '#fff', border: '1px solid var(--grey5)', borderRadius: 6, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={6} style={{ padding: 48, textAlign: 'center', color: 'var(--grey3)', fontSize: 13 }}>
